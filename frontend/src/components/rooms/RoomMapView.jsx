...
const RoomMapView = ({ room, height = '400px', showControls = true, isAdmin = false }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`flex items-center gap-3 px-6 py-4 rounded-2xl border transition-all ${isExact
        ? 'bg-green-50/50 border-green-100 text-green-700'
        : 'bg-blue-50/50 border-blue-100 text-blue-700'
        }`}>
        {isExact ? <MapPin size={18} /> : <Info size={18} />}
        <div className="flex-1">
          <p className="text-[10px] font-black uppercase tracking-widest mb-0.5">Location Status</p>
          <p className="text-sm font-bold truncate">
            {isExact ? 'Exact door-step location shared' : 'Approximate neighborhood location'}
          </p>
        </div>
        {!isExact && (
          <div className="px-3 py-1 bg-white rounded-lg text-[9px] font-black uppercase tracking-tight border border-blue-200">
            Privacy Protected
          </div>
        )}
      </div>
      ...
      {/* Map */}
      <div className="rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm relative" style={{ height }}>
        {/* Map Type Toggle */}
        ...
        <div className="p-8 bg-white border border-gray-100 rounded-[2.5rem] shadow-sm">
          ...
          {room.location.landmark && (
            <div className="mt-4 pt-4 border-t border-gray-50 flex items-center gap-2 text-blue-600">
              <MapPin size={16} />
              <span className="text-sm font-black uppercase tracking-widest">Near {room.location.landmark}</span>
            </div>
          )}
        </div>
      )}
